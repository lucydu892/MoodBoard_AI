...
export class DashboardComponent implements OnInit {
  ...
  getColorPalette(): void {
    const inputMood = this.mood || 'neutral'; // Standardwert, falls keine Stimmung gesetzt ist
    axios
  ...
  private fetchMoodColors(): void {
    axios.get('http://localhost:3000/colors')
      .then((response) => {
        console.log('✅ Farben geladen:', response.data);
      })
      .catch((error) => {
        console.error('Fehler beim Laden der Farben:', error.message);
      });
  }
}
